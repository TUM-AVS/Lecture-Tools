import numpy as np
import matplotlib
matplotlib.rcdefaults()           # Alle User-Styles zurücksetzen → weiß
...
BG_VERKETTUNG = "#FFF0F8"   # helles Lavendel   → Verkettungsmodus
...
class TransformationTool:
    ...
    def _on_mode_change(self, label):
        if "Verkettung" in label:
            self.mode = "Verkettung"
        elif "Aktiv" in label and "Vergleich" not in label:
            self.mode = "Aktiv"
    ...
    def _cleanup_chain(self):
        """Verkettung-Axes, Buttons und Titel entfernen."""
        for a in self._chain_axes:
            try:
                a.remove()
            except Exception:
                pass
        self._chain_axes    = []
        self._chain_buttons = []
        if self._chain_title is not None:
            try:
                self._chain_title.remove()
            except Exception:
                pass
            self._chain_title = None
    ...
    def _update(self, val):
        ...
        if self.mode == "Aktiv":
            ...
            bg = BG_PASSIV
        elif self.mode == "Vergleich":
            bg = BG_VERGLEICH
        else:  # Verkettung
            bg = BG_VERKETTUNG
        self.fig.set_facecolor(bg)
        self._draw_2d()
        if self.mode not in ("Vergleich", "Verkettung"):
            self._draw_3d()
            self._draw_matrix()
            self._draw_legend()
        if self.mode == "Verkettung":
            self._draw_chained()
    ...
    def _draw_2d(self):
        ...
        elif self.mode == "Verkettung":
            # Verkettung: alles ausblenden – _draw_chained() übernimmt
            self._cleanup_cmp()
    ...
    # ─────────────────────────────────────────────────
    #  VERKETTUNG: Intrinsisch (ZYX) vs. Extrinsisch (XYZ)
    # ─────────────────────────────────────────────────
    def _draw_chained(self):
        """Verkettete Rotationen: Intrinsisch vs. Extrinsisch – schrittweise."""
        self._cleanup_chain()
        self._cleanup_cmp()

        # Standard-Panels ausblenden
        self.ax_2d.set_visible(False)
        self.ax_3d.set_visible(False)
        self.ax_mat.set_visible(False)
        self.ax_leg.set_visible(False)

        # Gesamtfläche (identisch zum Vergleich-Modus)
        pos_2d  = self.ax_2d.get_position()
        pos_3d  = self.ax_3d.get_position()
        pos_mat = self.ax_mat.get_position()
        x0 = pos_2d.x0;  x1 = pos_3d.x1
        y0 = pos_mat.y0; y1 = pos_2d.y1
        W  = x1 - x0;   H  = y1 - y0

        gap    = 0.018
        half_w = (W - gap) / 2

        # Höhenaufteilung (von unten nach oben)
        btn_h  = 0.040
        form_h = 0.148
        ax3d_h = H - btn_h - form_h - 2 * gap

        y_btn  = y0
        y_form = y_btn  + btn_h  + gap
        y_3d   = y_form + form_h + gap

        # ── Zwei 3D-Axes nebeneinander ──
        ax_i = self.fig.add_axes([x0,              y_3d, half_w, ax3d_h], projection="3d")
        ax_e = self.fig.add_axes([x0 + half_w + gap, y_3d, half_w, ax3d_h], projection="3d")

        # ── Formel-Panel ──
        ax_f = self.fig.add_axes([x0, y_form, W, form_h], facecolor=PANEL_INFO)
        ax_f.axis("off")

        # ── Schritt-Buttons ──.  #in meherer schritte aufteilen um die einzelnen Transoformatione zu veranacchsualichen
        bw = W / 5.5
        bh = btn_h * 0.80
        ax_prev  = self.fig.add_axes([x0,              y_btn, bw * 1.4, bh])
        ax_lbl   = self.fig.add_axes([x0 + bw * 1.7,  y_btn, bw * 2.0, bh])
        ax_next  = self.fig.add_axes([x0 + bw * 4.0,  y_btn, bw * 1.4, bh])

        btn_prev = Button(ax_prev, "◀  Zurück",             color="#E0E8F0")
        btn_lbl  = Button(ax_lbl,  f"Schritt  {self.chain_step} / 3", color="#F5F5F5")
        btn_next = Button(ax_next, "Weiter  ▶",             color="#E0E8F0")

        for b in (btn_prev, btn_lbl, btn_next):
            b.label.set_fontsize(9)
        btn_lbl.label.set_color(TUM_GRAY)

        def on_prev(event):
            self.chain_step = max(0, self.chain_step - 1)
            self._update(None)

        def on_next(event):
            self.chain_step = min(3, self.chain_step + 1)
            self._update(None)

        btn_prev.on_clicked(on_prev)
        btn_next.on_clicked(on_next)

        self._chain_axes    = [ax_i, ax_e, ax_f, ax_prev, ax_lbl, ax_next]
        self._chain_buttons = [btn_prev, btn_lbl, btn_next]

        # ── Rotationsmatrizen je Schritt ──
        θz, θy, θx = self.rot_z, self.rot_y, self.rot_x
        step = self.chain_step

        # Intrinsisch ZYX: jede Achse dreht sich MIT dem Körper
        intr_R = [
            np.eye(3),
            Rz(θz),
            Rz(θz) @ Ry(θy),
        ...
        intr_R_prev = [None, np.eye(3), Rz(θz), Rz(θz) @ Ry(θy)]
        intr_angle  = [0, θz, θy, θx]          # Winkel des aktuellen Schritts
        intr_axis = [
            None,
            np.array([0., 0., 1.]),                   # Welt-Z (noch gleich Körper-Z)
            Rz(θz) @ np.array([0., 1., 0.]),           # Körper-Y nach Schritt 1
            Rz(θz) @ Ry(θy) @ np.array([1., 0., 0.]), # Körper-X nach Schritt 2
        ]
        intr_axis_lbl = [None, "Z (Körper)", "Y' (neu)", "X'' (neu)"]

        # Extrinsisch XYZ: jede Achse bleibt im Weltrahmen
        extr_R = [
            np.eye(3),
            Rx(θx),
            Ry(θy) @ Rx(θx),
        ...
        extr_R_prev = [None, np.eye(3), Rx(θx), Ry(θy) @ Rx(θx)]
        extr_angle  = [0, θx, θy, θz]
        extr_axis = [
            None,
            np.array([1., 0., 0.]),  # Welt-X (fix)
            np.array([0., 1., 0.]),  # Welt-Y (fix)
            np.array([0., 0., 1.]),  # Welt-Z (fix)
        ]
        extr_axis_lbl = [None, "X (Welt)", "Y (Welt)", "Z (Welt)"]

        # ── 3D-Frames zeichnen ──
        self._draw_chain_frame(
            ax_i, intr_R[step], intr_R_prev[step], intr_angle[step],
            intr_axis[step], intr_axis_lbl[step],
            f"INTRINSISCH  (ZYX – Körper-fest)   Schritt {step}/3", TUM_BLUE)
        self._draw_chain_frame(
            ax_e, extr_R[step], extr_R_prev[step], extr_angle[step],
            extr_axis[step], extr_axis_lbl[step],
            f"EXTRINSISCH  (XYZ – Welt-fest)   Schritt {step}/3", TUM_ORANGE)

        # ── Formelfeld ──
        self._draw_chain_formulas(ax_f, step, θz, θy, θx)

        # ── Kopftitel ──
        self._chain_title = self.fig.text(
            x0 + W / 2, y1 + 0.005,
            "◀  INTRINSISCH (Körper-fest, ZYX)  |  EXTRINSISCH (Welt-fest, XYZ)  ▶",
            ha="center", va="bottom",
            fontsize=11, fontweight="bold", color=TUM_BLUE)

    # ─────────────────────────────────────────────────
    def _draw_cube(self, ax, R, color,
                   scale=0.72, alpha_face=0.13, alpha_edge=0.60):
        """Rotierter Würfel (Kanten + halbtransparente Flächen)."""
        s = scale
        v = np.array([
            [-s, -s, -s], [ s, -s, -s], [ s,  s, -s], [-s,  s, -s],
            [-s, -s,  s], [ s, -s,  s], [ s,  s,  s], [-s,  s,  s],
        ])
        v = (R @ v.T).T   # Rotation anwenden

        edges = [(0,1),(1,2),(2,3),(3,0),   # Boden
                 (4,5),(5,6),(6,7),(7,4),   # Deckel
                 (0,4),(1,5),(2,6),(3,7)]   # Vertikale

        for i, j in edges:
            ax.plot(*zip(v[i], v[j]), color=color, lw=1.6, alpha=alpha_edge, zorder=3)
        ...
        faces = [[v[i] for i in f] for f in faces_idx]
        poly  = Poly3DCollection(faces, alpha=alpha_face,
                                 facecolor=color, edgecolor="none")
        ax.add_collection3d(poly)

    # ─────────────────────────────────────────────────
    def _draw_rotation_arc(self, ax, rot_axis, ref_vec, angle_deg, color, r=1.2):
        """Kreisbogen in 3D um rot_axis, Startpunkt = Projektion von ref_vec."""
        n = rot_axis / np.linalg.norm(rot_axis)
        # Projektion senkrecht zur Achse
        v = ref_vec - np.dot(ref_vec, n) * n
        if np.linalg.norm(v) < 1e-6:
            for cand in [np.array([1.,0.,0.]), np.array([0.,1.,0.]), np.array([0.,0.,1.])]:
                v = cand - np.dot(cand, n) * n
                if np.linalg.norm(v) > 1e-6:
                    break
        v = v / np.linalg.norm(v) * r
        w = np.cross(n, v)
        if np.linalg.norm(w) > 1e-6:
            w = w / np.linalg.norm(w) * r

        sign = 1 if angle_deg >= 0 else -1
        t = np.linspace(0, np.radians(abs(angle_deg)), 60)
        pts = np.array([v * np.cos(a) + w * np.sin(a) * sign for a in t])

        ax.plot(pts[:, 0], pts[:, 1], pts[:, 2],
                color=color, lw=2.0, alpha=0.85, zorder=5)
        # Pfeil am Ende des Bogens
        if len(pts) >= 2:
            tip = pts[-1]
            d   = pts[-1] - pts[-2]
            d   = d / np.linalg.norm(d) * 0.14
            ax.quiver(*tip, *d, color=color,
                      arrow_length_ratio=1.0, linewidth=0.5,
                      alpha=0.88, zorder=6)

    # ─────────────────────────────────────────────────
    def _draw_chain_frame(self, ax, R, R_prev, rot_angle,
                          rot_axis, rot_ax_lbl, title, color):
        """Weltrahmen (grau) + Geist-Achsen (vorheriger Schritt) +
        aktuelle Körper-Achsen (farbig) + Würfel + Rotationsbogen."""
        ax.cla()
        sc  = 1.4
        lim = 2.2
        ...
        ax.set_facecolor("#F8F9FF")
        ax.set_title(title, fontsize=8.5, color=color, pad=4,
                     fontweight="bold")

        # ── Geist-Würfel: Ausgangsposition (grau, sehr transparent) ──
        is_rotated = not np.allclose(R, np.eye(3))
        if is_rotated:
            self._draw_cube(ax, np.eye(3), color="0.60",
                            alpha_face=0.04, alpha_edge=0.18)

        # ── Rotierter Würfel (farbig) ──
        self._draw_cube(ax, R, color=color,
                        alpha_face=0.13, alpha_edge=0.60)

        # ── Welt-Achsen (dünn grau, IMMER sichtbar als Referenz) ──
        for vec0, lbl, ls in [
            (np.array([sc, 0., 0.]), "$X_W$", "-"),
            (np.array([0., sc, 0.]), "$Y_W$", "--"),
            (np.array([0., 0., sc]), "$Z_W$", ":"),
        ]:
            ax.quiver(0, 0, 0, *vec0, color="0.55",
                      arrow_length_ratio=0.18, linewidth=1.3,
                      linestyle=ls, alpha=0.55)
            ax.text(*(vec0 * 1.18), lbl, color="0.45", fontsize=7, alpha=0.65)

        # ── Geist-Achsen: vorheriger Schritt (grau/transluzent) ──
        if R_prev is not None and not np.allclose(R_prev, R):
            for vec0 in [np.array([sc,0.,0.]), np.array([0.,sc,0.]), np.array([0.,0.,sc])]:
                vec = R_prev @ vec0
                ax.quiver(0, 0, 0, *vec, color="0.60",
                          arrow_length_ratio=0.18, linewidth=1.6,
                          alpha=0.30, linestyle="-")

        # ── Aktuelle Körper-Achsen (farbig, kräftig) ──
        for vec0, lbl in [
            (np.array([sc, 0., 0.]), "X"),
            (np.array([0., sc, 0.]), "Y"),
            (np.array([0., 0., sc]), "Z"),
            ...
            ax.quiver(0, 0, 0, *vec, color=color,
                      arrow_length_ratio=0.20, linewidth=2.5, alpha=1.0)
            ax.text(*(vec * 1.16), lbl, color=color,
                    fontsize=9, fontweight="bold")

        # ── Rotationsachse (rot, bidirektional gestrichelt) ──
        if rot_axis is not None:
            ra = rot_axis / np.linalg.norm(rot_axis) * sc * 1.05
            ax.quiver(0, 0, 0,  *ra, color="crimson",
                      arrow_length_ratio=0.10, linewidth=2.0,
                      alpha=0.82, linestyle="--")
            ax.quiver(0, 0, 0, *(-ra), color="crimson",
                      arrow_length_ratio=0.0,  linewidth=2.0,
                      alpha=0.82, linestyle="--")
            ax.text(*(ra * 1.10), f" {rot_ax_lbl}", color="crimson",
                    fontsize=7.5, fontweight="bold",
                    bbox=dict(facecolor="white", edgecolor="none", alpha=0.70, pad=1))

            # ── Rotationsbogen (Kreisbogen mit Pfeil) ──
            if abs(rot_angle) > 0.5:
                # Startvektor: vorherige Körper-X Achse, senkrecht zur Rotationsachse
                ref = (R_prev @ np.array([0., 1., 0.])
                       if R_prev is not None else np.array([0., 1., 0.]))
                self._draw_rotation_arc(ax, rot_axis, ref, rot_angle,
                                        color="crimson", r=sc * 0.88)

        ax.scatter(0, 0, 0, color=color, s=55, zorder=6,
                   edgecolors="white", linewidths=0.8)

    # ─────────────────────────────────────────────────
    def _draw_chain_formulas(self, ax, step, θz, θy, θx):
        """Formelfeld für Verkettungs-Modus."""
        intr_f = [
            r"$R_{intr} = I$   (keine Rotation)",
            f"$R_{{intr}} = R_z(\\theta_z={θz:.0f}°)$",
            f"$R_{{intr}} = R_z(\\theta_z) \\cdot R_{{y'}}(\\theta_y={θy:.0f}°)$"
            r"     [$y'$ = neues Körper-Y nach Schritt 1]",
            f"$R_{{intr}} = R_z(\\theta_z) \\cdot R_{{y'}}(\\theta_y) \\cdot R_{{x''}}(\\theta_x={θx:.0f}°)$",
        ]
        extr_f = [
            r"$R_{extr} = I$   (keine Rotation)",
            f"$R_{{extr}} = R_x(\\theta_x={θx:.0f}°)$",
            f"$R_{{extr}} = R_y(\\theta_y={θy:.0f}°) \\cdot R_x(\\theta_x)$"
            r"     [$y$ = festes Welt-Y]",
            f"$R_{{extr}} = R_z(\\theta_z={θz:.0f}°) \\cdot R_y(\\theta_y) \\cdot R_x(\\theta_x)$",
        ]
        notes = [
            "Beide Methoden starten ohne Rotation – Körper liegt im Weltrahmen.",
            f"Intrinsisch: 1. Rotation um Körper-Z ($\\theta_z={θz:.0f}°$)  |  "
            f"Extrinsisch: 1. Rotation um Welt-X ($\\theta_x={θx:.0f}°$)",
            f"Intrinsisch dreht um das neu ausgerichtete Körper-Y ($\\theta_y={θy:.0f}°$)  |  "
            f"Extrinsisch dreht um festes Welt-Y ($\\theta_y={θy:.0f}°$)",
            r"$\mathbf{Schlüssel\text{-}Erkenntnis:}$   "
            r"Intrinsisch ZYX  $\equiv$  Extrinsisch XYZ  $\Longrightarrow$  "
            r"$\mathbf{gleiche\ Endmatrix\ R}$!",
        ]

        # ── Linke Spalte: Intrinsisch ──
        ax.text(0.02, 0.97, "Intrinsisch (Körper-fest):", transform=ax.transAxes,
                fontsize=9, fontweight="bold", color=TUM_BLUE, va="top")
        ax.text(0.02, 0.74, intr_f[step], transform=ax.transAxes,
                fontsize=9, color=TUM_BLUE, va="top")
        ax.text(0.02, 0.50,
                r"Rotationen wirken im mitrotierenden Frame",
                transform=ax.transAxes, fontsize=7.8, color=TUM_BLUE,
                va="top", style="italic", alpha=0.80)

        # ── Rechte Spalte: Extrinsisch ──
        ax.text(0.52, 0.97, "Extrinsisch (Welt-fest):", transform=ax.transAxes,
                fontsize=9, fontweight="bold", color=TUM_ORANGE, va="top")
        ax.text(0.52, 0.74, extr_f[step], transform=ax.transAxes,
                fontsize=9, color=TUM_ORANGE, va="top")
        ax.text(0.52, 0.50,
                r"gleiche Matrix = inverse Reihenfolge auf festen Achsen",
                transform=ax.transAxes, fontsize=7.8, color=TUM_ORANGE,
                va="top", style="italic", alpha=0.80)

        # ── Hinweis-Box unten ──
        is_final = (step == 3)
        if is_final:
            note_txt = (r"$R = R_z(\psi)\,R_y(\theta)\,R_x(\phi)$   "
                        r"$\Longrightarrow$   Intrinsisch ZYX $\equiv$ Extrinsisch XYZ")
            fc, ec, fw, fsz, clr = "#FFDDE8", TUM_ORANGE, "bold", 9.5, TUM_ORANGE
        else:
            note_txt = notes[step]
            fc, ec, fw, fsz, clr = "#EEF3F9", TUM_GRAY, "normal", 8.5, TUM_BLUE
        ax.text(0.50, 0.33, note_txt, transform=ax.transAxes,
                fontsize=fsz, color=clr, ha="center", va="top", fontweight=fw,
                bbox=dict(boxstyle="round,pad=0.4",
                          facecolor=fc, edgecolor=ec, alpha=0.93))
